...
class Question(models.Model):
    ...
    GRIDIN_SINGLE_ANSWER = 'SINGLE_ANSWER'
    GRIDIN_MULTI_ANSWER = 'MULTI_ANSWER'
    GRIDIN_RANGE_BASED_ANSWER = 'RANGE_BASED_ANSWER'
    MCQ_SINGLE_CHOICE_QUESTION = 'SINGLE_CHOICE'
    MCQ_MULTI_CHOICE_QUESTION = 'MULTI_CHOICE'
    MCQ_READING_COMPREHENSION = 'READING_COMPREHENSION'

    FILL_IN_BLANKS = 'FILL_IN_BLANKS'
    QUESTION_SUBTYPE = [
        (MCQ_SINGLE_CHOICE_QUESTION, 'Single Choice'),
        (MCQ_MULTI_CHOICE_QUESTION, 'Multi Choice'),
        (FILL_IN_BLANKS, 'Fill in the Blanks'),
        (MCQ_READING_COMPREHENSION, 'Reading Comprehension'),
        (GRIDIN_SINGLE_ANSWER, 'Single Answer'),
        (GRIDIN_MULTI_ANSWER, 'Multi Answer'),
        (GRIDIN_RANGE_BASED_ANSWER, 'Range Based Answer'),
    ]

    question_type = models.CharField(max_length=30, choices=QUESTION_TYPE, default=MCQ)
    question_subtype = models.CharField(max_length=30, choices=QUESTION_SUBTYPE, default=MCQ_SINGLE_CHOICE_QUESTION)
    ...
    def convert_to_decimal(cls, value):
        if '/' in value:
            ...
            try:
                ...
                return Decimal(numerator_decimal / denominator_decimal)
            except InvalidOperation:
                raise ValueError(f"Invalid literal for Decimal: '{value}'")
        else:
    ...
    @classmethod
    def compare_answers(cls, answer, options, decimal_places=5, tolerance=Decimal('1e-2')):
        answer_decimal = cls.convert_to_decimal(answer).quantize(Decimal('1.' + '0' * decimal_places),
                                                                 rounding=ROUND_HALF_UP)

        for option in options:
            option_decimal = cls.convert_to_decimal(option).quantize(Decimal('1.' + '0' * decimal_places),
                                                                     rounding=ROUND_HALF_UP)
            if abs(answer_decimal - option_decimal) <= tolerance:
                return True
        return False
...
class QuestionLog(models.Model):
    ...
    question = models.ForeignKey(Question, on_delete=models.CASCADE, related_name='logs')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='question_logs')
    action = models.CharField(max_length=10, choices=ACTION_CHOICES)
    timestamp = models.DateTimeField(auto_now_add=True)
    ip_address = models.GenericIPAddressField(null=True, blank=True)

    def __str__(self):
        return f"{self.user.username} {self.get_action_display()} Q{self.question.id} on {self.timestamp}"
