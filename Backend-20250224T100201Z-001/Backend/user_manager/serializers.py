...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_faculty_details(self, obj):
        if obj.role.name == 'student':
            student_metadata = self._get_student_metadata(obj)
            if student_metadata:
                return UserDetailSerializer(student_metadata.faculties.all(), many=True).data
        return None
...
class UserCreationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'email', 'phone_number', 'alternative_number', 'name',
            'role', 'courses', 'address', 'dob', 'blood_group']
    ...
    def create(self, validated_data):
        with transaction.atomic():
            ...
            alternative_number = validated_data.pop('alternative_number', None) 
...
class ApproveStudentSubscriptionSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if data['is_temp_user']:
            if 'father_id' not in data and 'father_email' not in data and \
               'mother_id' not in data and 'mother_email' not in data:
        ...
        if 'courses' in validated_data:
            ...
            for enrollment_data in validated_data['courses']:
                ...
                CourseEnrollment.objects.create(
                    student=instance,
                    course=course,
                    subscription_start_date=enrollment_data['subscription_start_date'],
                    subscription_end_date=enrollment_data['subscription_end_date'],
                    subscription_type=enrollment_data['subscription_type']
                )
