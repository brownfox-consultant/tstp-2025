...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    dob = models.DateField(blank=True, null=True)      # <- new field
    blood_group = models.CharField(max_length=5, blank=True, null=True)  # <- new field
...
class StudentMetadata(models.Model):
    ...
    faculties = models.ManyToManyField(User, related_name="faculty_students", blank=True)
    ...
    @classmethod
    def create_metadata(cls, faculties=None, **kwargs):
        metadata_instance = cls.objects.create(**kwargs)
        if faculties:
            metadata_instance.faculties.set(faculties)
        return metadata_instance


    def update_metadata(self, faculties=None, **kwargs):
