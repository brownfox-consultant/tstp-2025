                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {modalData.options?.map(({ description, is_correct }, index) => {
                         // selectedListQuestion from list has the user's selections as array of indices?
                         // We need to match it. ReportTable uses selectedOptions state found from question.
                         // But map expects indices.
                         // Let's rely on selectedListQuestion.selected_options which comes from the LIST API (which we know has selections)
                         // OR modalData might NOT have selections?
                         // ReportTable: `const isSelected = (selectedOptions ?? []).includes(index);`
                         // where selectedOptions was `selectedQuestion.selected_options`.
                         // Let's use `selectedListQuestion.selected_options`.
