...
const ReportNew = ({ testSubmissionId, onClose }) => {
  ...
  useEffect(() => {
    if (resultData?.subjects?.length > 0) {
      ...
      if (
        activeTab === "english" &&
        !subjectNames.some((n) => n.includes("english"))
      ) {
  ...
  const getSubjectIndex = (subjectName) => {
    return resultData?.subjects?.findIndex((s) =>
      s.name.toLowerCase().includes(subjectName)
    );
  };
  ...
  const mergeAreasOfFocus = (subjectName) => {
    ...
    subject?.sections?.forEach((section) => {
      ...
      Object.entries(focus).forEach(([topic, data]) => {
        ...
        combined[topic].correct +=
          data.correct_count || 0;

        combined[topic].incorrect +=
  ...
  /* ============================================================
     ...
     Examples:
     English + Section A = Eng A
     English + Section B = Eng B
     Math + Section A    = Math A
     Math + Section B    = Math B
  ============================================================ */

  const getCompactSectionLabel = (
    subjectName,
    sectionName
  ) => {
    ...
    */
    ...
    return `${subjectShort} ${sectionShort}`.trim();
  };

  /* ============================================================
     BUILD SINGLE-LINE SUBJECT + SECTION TABS
     
     Eng A | Eng B | Math A | Math B
  ============================================================ */

  const getQuestionSectionTabs = () => {
    const tabs = [];
  ...
  /* ============================================================
     ...
     IMPORTANT:
     - Every question starts from the same bottom baseline.
     - More seconds = taller bar.
     - Less seconds = shorter bar.
     ...
     This gives the visual comparison requested.
  ============================================================ */
      ...
      return (
        <div className="text-center py-12 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Complete the test with navigation tracking enabled to see insights.
          </p>
        </div>
      );
    }
          ...
          onClick={() => {
            if (onClose) {
              ...
              if (
                ...
                router.push(
                  `/tstp/student/${params.id}/test/full`
                );
              } else {
                router.back();
              }
                  ...
                  (tab) => {
                    ...
                    return (
                      <button
                        ...
                        title={`${tab.subjectName} - ${tab.sectionName}`}
                        onClick={() => {
                          setQuestionMainTab(
                            tab.subjectName
                          );
                        ...
                        className={`flex-shrink-0 min-w-[76px] px-4 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 border ${
                          isActive
                            ? "bg-gradient-to-r from-primary-color to-orange-500 text-white border-transparent shadow-md shadow-orange-200"
                            : "bg-white border-gray-200 text-gray-700 hover:bg-gray-50 hover:border-orange-200 hover:text-orange-600"
                        }`}
                      >
                        {
                          tab.label
