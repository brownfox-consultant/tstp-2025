...
function DashboardLayout({ children }) {
  ...
  const tab = pathParts[3]; // Accessing the tab part correctly for mentor routes
  ...
  const handleProfileClick = useCallback(() => {
    // Assuming profile is at /mentor/[id] or a specific profile route
    // The original code had a dropdown, we'll route to the base path for now or could implement specific profile route
    const newPath = pathname.split("/");
    router.push(`${newPath.slice(0, 4).join("/")}`);
  }, [pathname, router]);
  ...
  const handleMenuClick = useCallback((key, disabled) => {
    if (disabled) return;
    router.push(`/tstp/mentor/${id}/${key}`);
    if (isMobile) {
